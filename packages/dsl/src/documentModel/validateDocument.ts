...
const REMOVED_WRAPPER_ELEMENT_TYPES = new Set(['sequence', 'fadein', 'fadeout', 'draw', 'write', 'transform', 'morph', 'stagger', 'parallel']);
const REMOVED_ANIMATION_PROPS = new Set(['fadeIn', 'fadeOut', 'draw', 'write', 'easing']);
...
function validateElement(id: string, element: ElucimElement, errors: ValidationError[]) {
  ...
  } else if (REMOVED_WRAPPER_ELEMENT_TYPES.has(element.type.toLowerCase())) {
    errors.push({
      path: `${path}.type`,
      message: `Removed wrapper element "${element.type}" is not part of Elucim Documents. Use timelines and state machines for motion.`,
      severity: 'error',
    });
  }
  if (!element.props || typeof element.props !== 'object' || Array.isArray(element.props)) {
    ...
    for (const prop of REMOVED_ANIMATION_PROPS) {
      if (prop in element.props) {
        errors.push({
          path: `${path}.props.${prop}`,
          message: `Removed animation prop "${prop}" is not part of Elucim Documents. Use timeline tracks and keyframes instead.`,
          severity: 'error',
        });
      }
    }
