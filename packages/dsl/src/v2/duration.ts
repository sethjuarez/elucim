import type { ElucimV2Document, ElucimV2StateMachine, ElucimV2Timeline } from './types';

export const DEFAULT_LINEAR_DURATION_IN_FRAMES = 120;

export function getMaxTimelineDuration(timelines: Record<string, ElucimV2Timeline> | undefined): number | undefined {
...
export function getDocumentLinearDuration(doc: Pick<ElucimV2Document, 'timelines' | 'stateMachines' | 'defaultStateMachine'>): number {
...
function getDefaultStateMachineInitialTimelineDuration(doc: Pick<ElucimV2Document, 'timelines' | 'stateMachines' | 'defaultStateMachine'>): number | undefined {
...
function getInitialStateId(machine: ElucimV2StateMachine): string | undefined {
...
export type ElucimV2ExportPolicy =
...
export function resolveExportFrameCount(doc: ElucimV2Document, policy: ElucimV2ExportPolicy): number {
  switch (policy.type) {
    case 'timeline': {
...
function requireMachine(doc: ElucimV2Document, machineId: string) {
