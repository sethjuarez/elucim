...
- Add explicit relationship metadata (`target`, `flowFrom`, `flowTo`, `relationship`, `group`, `rank`) when element placement should communicate a flow or explanation.
...
npx elucim layout diagram.elc --out laid-out.elc --json
```

`ops --json` returns the CLI command catalog and the same code-backed agent operation catalog exposed by `getAgentOperationCatalog()`.
...
| Presets | `createCalloutCardPreset` | Start from semantic, theme-tokenized explanatory building blocks instead of raw shapes. |
...
For explanatory structure, prefer semantic presets:
...
const calloutElements = createCalloutCardPreset({
  ...
  y: 420,
  title: 'Key insight',
  body: 'Use a callout when the diagram needs an explicit takeaway.',
});
```
