...
`ops --json` returns the CLI command catalog and the same code-backed agent operation catalog exposed by `getAgentOperationCatalog()`. If installed globally, the binary is still `elucim`.
...
9. **Polish** with `report.polish`, `suggestDocumentNudges()`, and `applyNudge()`.
10. **Explain** remaining warnings and return the final document.
...
| Semantic motion | `planMotionBeats`, `createSemanticMotionTimeline`, `createAutoStaggerTimeline`, `createStateSnapshotMotion`, `lintMotion`, `previewBeatDiffs`, `createReducedMotionDocument`, `holdFinalFrame` | Compile intent-level verbs and narration beats into ordinary v2 timelines/state machines, then verify readability and accessibility. |
...
| Quality | `evaluateSceneForAgent`, `inspectSceneForAgent`, `analyzePolish`, `inspectPolishHeuristics` | Catch missing metadata, missing motion, blank frames, tiny scenes, off-canvas elements, zero-size elements, low contrast, graph readability issues, weak hierarchy, and static timelines. `inspectPolishHeuristics` exposes the raw evidence behind the score: bounds, intersections, overflow, text, colors, graph details, connector continuations, and semantic relationships. |
...
| Presets | `createCalloutCardPreset`, `createConnectorPreset`, `createTextBlockPreset`, `createStepCardPreset`, `createCardGridPreset`, `createDecisionNodePreset`, `createBoundaryPreset`, `createBadgePreset`, `createQueueStackPreset`, `createTimelineRoadmapPreset`, `createComparisonTablePreset`, `createAutoLayoutGroupPreset`, `createProgressiveRevealGroupPreset` | Start from semantic, theme-tokenized explanatory building blocks instead of raw shapes. |
...
`evaluateSceneForAgent(doc)` also includes `quality.polish`, so agents can return one quality object with validation, issues, nudges, summary, and polish diagnostics.
...
For explanatory structure, prefer semantic presets. They emit ordinary editable v2 elements, groups, and timelines, not opaque custom schema nodes.
