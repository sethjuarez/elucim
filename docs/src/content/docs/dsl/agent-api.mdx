...
- Do not use React wrapper animation syntax or removed element types such as `fadeIn`, `draw`, `sequence`, `stagger`, or `parallel`.
...
1. **Create** a normalized starter document with `createDocument()`, or start text-heavy scenes from `createTextCalloutScenePreset()`, `createThreeCardFlowScenePreset()`, or `createComparisonScenePreset()` and wrap them with `createAgentSafeDocument()`. For calculus explanations, start from `createCalculusDerivativeScenePreset()`, `createCalculusRiemannScenePreset()`, or `createCalculusAccumulationScenePreset()` so the lesson uses semantic math primitives instead of raw geometry.
...
let doc = createAgentSafeDocument(derivativeScene, {
  metadata: {
    title: 'Derivative intuition',
    intent: 'Show how secant slope approaches tangent slope.',
  },
});

doc = addRevealTimeline(doc, {
  id: 'intro',
  targets: ['derivative-title', 'derivative-axes', 'derivative-secant', 'derivative-tangent', 'derivative-explain'],
...
| Presets | `createTextCalloutScenePreset`, `createThreeCardFlowScenePreset`, `createComparisonScenePreset`, `createCalculusDerivativeScenePreset`, `createCalculusRiemannScenePreset`, `createCalculusAccumulationScenePreset`, `createAgentSafeDocument`, `createConnectorPreset`, `createTextBlockPreset`, `createTextBoxPreset`, `createStepCardPreset`, `createCardGridPreset`, `createDecisionNodePreset`, `createBoundaryPreset`, `createBadgePreset`, `createQueueStackPreset`, `createTimelineRoadmapPreset`, `createComparisonTablePreset`, `createAutoLayoutGroupPreset`, `createProgressiveRevealGroupPreset` | Start from semantic, theme-tokenized explanatory building blocks instead of raw shapes. The agent-safe scene presets use bounded textboxes for generated copy by construction, and the calculus presets use semantic math primitives for derivative, Riemann sum, and accumulation lessons. |
