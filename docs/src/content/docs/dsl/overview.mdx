...
The `@elucim/dsl` package lets agents, content pipelines, and React applications describe Elucim visuals as normalized **JSON, YAML, or canonical JSX documents**. Elucim documents are normalized scene graphs with explicit elements, timelines, and optional state machines.
...
- [Canonical React authoring](/dsl/react-authoring/) — author the same document model with JSX
