...
const doc = applyAgentCommands(createDocument({
...
}), [
  {
    ...
    element: {
      ...
      type: 'text',
      role: 'title',
      intent: { purpose: 'Introduce the takeaway' },
      layout: { x: 120, y: 120 },
...
```ts
import { applyNudge, suggestDocumentNudges } from '@elucim/dsl/agent';

const nudges = suggestDocumentNudges(doc);
const polished = applyNudge(doc, nudges[0]).document;
```

Initial nudges cover safe metadata polish and a reviewable timeline-based intro clip. This gives agents a simple workflow: generate the 80% layout, ask Elucim for refinements, then apply or explain the chosen command-backed nudges.
