...
Use composite helpers when an agent needs polished explanatory structure but the user still needs editable pieces in the editor. These helpers emit normal v2 groups/primitives and optional timelines:
...
Agents should describe motion in beats and semantic verbs, then let Elucim compile those intents to v2 timelines/state machines. This is safer than asking a model to hand-compute every keyframe.
