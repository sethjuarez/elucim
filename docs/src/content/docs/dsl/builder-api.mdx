...
```ts
import { presentation, darkTheme, deckDarkTheme } from '@elucim/dsl';
...
For slide-like generated visuals, use `deckDarkTheme` or `deckLightTheme` with
the deck helper methods:

```ts
const deck = presentation('CutReady Visual', deckDarkTheme, { width: 960, height: 540 })
  .slide('Pipeline', s => {
    s.hero('From raw footage to ready clips');
    s.process(['Upload', 'Analyze', 'Export']);
    s.callout('One Elucim visual can become one full-screen slide.');
  })
  .build();
```
...
Deck helpers generate regular DSL nodes with polished defaults for presentation
slides and CutReady-style fullscreen visuals.

| Method | Description |
|--------|-------------|
| `hero(title, subtitle?, opts?)` | Add a large presentation title block with subtle background accents |
| `card(content, opts?)` | Add a polished card/panel. Options include `title`, `x`, `y`, `width`, `height`, `accent`, `fill`, `stroke`, `fontSize` |
| `callout(content, opts?)` | Add an emphasized takeaway panel near the bottom of the slide |
| `metric(value, label, opts?)` | Add a large metric/value card |
| `process(steps, opts?)` | Add a horizontal connected process layout |
| `compare(left, right, opts?)` | Add two comparison cards. `left`/`right` are `{ title, body }` |
...
const doc = presentation('Linear Algebra', darkTheme, {
  ...
  .slide('Vectors', s => {
    ...
    s.latex('\\vec{v} = \\begin{bmatrix} 3 \\\\ 2 \\end{bmatrix}', {
      x: 600, y: 300, fontSize: 28, color: t.foreground,
    });
