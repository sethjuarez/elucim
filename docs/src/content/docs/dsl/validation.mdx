...
:::note
`DslRenderer` calls document validation automatically and displays errors inline. The `fromYaml()` function also validates automatically and throws `ElucimYamlError` on failure. Call `validateDocument` directly when you want to check JSON documents outside of a React render.
:::
