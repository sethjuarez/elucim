...
The `<DslRenderer>` component takes an `ElucimDocument` and renders it as a fully interactive Elucim scene — no JSX authoring required. Documents can be authored as JSON objects or parsed from YAML with [`fromYaml()`](/dsl/overview/#yaml).
...
| `controls` | `boolean` | — | Override the document's `controls` setting (player scene only) |
| `autoPlay` | `boolean` | — | Override the document's `autoPlay` setting (player scene only) |
| `loop` | `boolean` | — | Override the document's `loop` setting (player scene only) |
...
const doc: ElucimDocument = {
  ...
  elements: {
    orb: {
      id: 'orb',
      type: 'circle',
      props: { type: 'circle', cx: 400, cy: 300, r: 60, stroke: '$accent', strokeWidth: 3, opacity: 0 },
    },
  },
  timelines: {
    intro: {
      id: 'intro',
      duration: 30,
      tracks: [{ target: 'orb', property: 'opacity', keyframes: [{ frame: 0, value: 0 }, { frame: 30, value: 1 }] }],
    },
  },
...
:::tip
Explicit `width`/`height` on `scene` override preset values.
:::
...
<DslRenderer dsl={{ version: '2.0', elements: {} } as any} />
...
:::tip
Combine `DslRenderer` with agent-generated JSON/YAML to render normalized documents without writing React scene code.
:::
