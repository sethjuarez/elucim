...
description: Low-level React API reference for the Player playback container.
---

The `<Player>` is the low-level React playback container used by Elucim's renderer and by advanced hand-coded React scenes. For authored documents, use a normalized `ElucimDocument` rendered with `<DslRenderer dsl={doc} />`; `DslRenderer` creates the appropriate player surface for you.
...
| `children` | `React.ReactNode` | — | Elucim primitives or custom React-rendered content |

## Basic Usage

```tsx
import { Player, Circle, Text, useCurrentFrame, interpolate } from '@elucim/core';
