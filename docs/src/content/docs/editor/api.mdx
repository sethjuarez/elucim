...
| Prop | Description |
|------|-------------|
| `initialDocument` | Starting document. Normalized v2 `ElucimDocument` is the public authoring path; legacy/renderable documents are accepted for compatibility. |
...
| `onDocumentChange` | Called after edits with normalized v2 output and compatibility details. Skips initial mount. |
| `onCompatibilityWarnings` | Called when normalized output has warnings such as renamed IDs, pruned timelines, or invalid v2 output. |
...
<EditorProvider initialDocument={doc}>
  <ElucimEditorLayout v2Document={doc} onDocumentChange={setDoc} />
  <CustomSaveButton />
</EditorProvider>;
```
