...
|------|------|---------|-------------|
| `initialDocument` | `ElucimDocument \| ElucimV2Document` | Empty 800×600 scene | Starting scene document. Normalized documents are the native authoring path. |
...
| `onV2DocumentChange` | `(doc: ElucimV2Document, details: ElucimV2EditorChangeDetails) => void` | — | Called with normalized output when editing a normalized document. Preserves metadata, intent, timelines, and state machines where possible. |
| `onV2CompatibilityWarnings` | `(warnings: string[]) => void` | — | Called when normalized output has warnings such as renamed/deleted element references or pruned timelines. |
...
### Native document authoring

The editor treats normalized documents as the primary authoring model. Normal edits emit a normalized document through `onV2DocumentChange`; ID renames update timeline track targets where the editor can map the old and new IDs, and deleted targets produce compatibility warnings.

The editor exposes document metadata and selected element intent fields in the `Polish` workspace, reviewable polish nudges from `suggestDocumentNudges()`, accept/dismiss controls for suggested nudges with command summaries and previewed command results, animation creation with editable timeline tracks/keyframes, and state-machine creation with state and transition authoring.

The editor shell uses progressive-disclosure workspaces (`Design`, `Animate`, `State Machine`, and `Polish`) plus collapsible/resizable left, inspector, and timeline regions. Normalized documents open in the `Animate` workspace so the animation list and selected animation timeline are visible immediately in a taller motion panel. The motion panel has compact `Animations` and `State machines` tabs so animation clips and state-machine graphs can be edited independently. Like a simplified Rive workflow, only one animation is expanded at a time; the timeline shows that animation's tracks and keyframes while the details pane owns numeric edits. The `State Machine` workspace opens the state-machine tab in the same bottom motion area: the graph shows an explicit draggable Entry node, compact draggable animation-state nodes, and labeled transition edges. Machine inputs are listed in the inspector, event transitions can be authored from a selected state or by dragging a connection, and selecting a transition reveals its trigger input, target state, and transition animation. State-machine graphs open in a fitted ranked layout capped at 1x zoom, can be reset between horizontal and vertical flow, and persist manual Entry position, state positions, and graph viewport in `stateMachines.*.layout`.

```tsx
<ElucimEditor
  initialDocument={v2Doc}
  onV2DocumentChange={(doc, details) => {
    saveV2(doc);
    if (details.warnings.length > 0) showWarnings(details.warnings);
  }}
  onV2CompatibilityWarnings={showWarnings}
/>
```
