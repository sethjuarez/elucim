...
The editor also exposes basic v2-native controls: document metadata and selected element intent fields in the States panel, reviewable polish nudges from `suggestDocumentNudges()`, accept/dismiss controls for suggested nudges with command summaries and previewed command results, simple timeline clip/keyframe editing, and simple state-machine transition editing.
