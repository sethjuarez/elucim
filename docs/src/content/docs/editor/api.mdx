...
The editor shell uses progressive-disclosure workspaces (`Design`, `Animate`, `State Machine`, and `Polish`) plus collapsible/resizable left, inspector, and timeline regions. Normalized documents open in the `Animate` workspace so the animation list and selected animation timeline are visible immediately in a taller motion panel. The motion panel has compact `Animations` and `State machines` tabs so animation clips and state-machine graphs can be edited independently. Like a simplified Rive workflow, only one animation is expanded at a time; the timeline shows that animation's tracks and keyframes while the details pane owns numeric edits. The `State Machine` workspace opens the state-machine tab in the same bottom motion area: states are draggable nodes that can play animation clips, transitions are labeled directed edges, and selecting a node or edge reveals the relevant state or transition controls. State-machine graphs open in a fitted ranked layout, can be reset between horizontal and vertical flow, hide reset transitions in favor of a machine-level reset-state picker, and persist manual node positions in `stateMachines.*.layout.states`.
