...
## 5. Presentation-Grade Talk Starter

Use the premium deck theme and high-level helpers when one generated visual should become one full-screen slide.

```tsx
import { DslRenderer, deckDarkTheme, presentation } from '@elucim/dsl';

const doc = presentation('NDC Talk Starter', deckDarkTheme, {
  width: 960,
  height: 540,
  transition: 'fade',
  showHud: false,
  ...
  .slide('Opening', s => {
    s.hero('From demo to deck', 'Elucim visuals as full-screen talk slides');
    s.callout('One generated visual = one slide-ready story beat.');
  })
  .slide('Talk arc', s => {
    s.title('Shape the talk as a sequence');
    s.process(['Problem', 'Mental model', 'Demo', 'Takeaway']);
  })
  .slide('Visual grammar', s => {
    ...
    s.metric('1', 'hero object');
  })
  .build();

function TalkPreview() {
  return <DslRenderer dsl={doc} poster="last" />;
}
```

Use `showHud: true` for an interactive preview deck. Keep it `false` for exported or poster-style frames that should drop cleanly into a talk without presentation chrome.
